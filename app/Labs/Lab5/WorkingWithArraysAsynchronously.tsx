...
export default function WorkingWithArraysAsynchronously() {
    ...
    return (
        <div id="wd-asynchronous-arrays">
            ...
            <h4>
                Todos
                <FaCirclePlus role="button" onClick={createNewTodo} className="text-success float-end fs-3" />
                <FaCirclePlus onClick={postNewTodo} className="text-info float-end fs-3 me-3" id="wd-post-todo" />
            </h4>
            <ListGroup>
                {todos.map((todo) => (
                    <ListGroupItem key={todo.id}>
                        <FaTrash onClick={() => removeTodo(todo)}
                            className="text-danger float-end mt-1" role="button" id="wd-remove-todo" />
                        <TiDelete onClick={() => deleteTodo(todo)}
                            className="text-danger float-end me-2 fs-3" role="button" id="wd-delete-todo" />
                        <FaPencil onClick={() => editTodo(todo)} role="button" className="text-info float-end me-2 mt-1" />
